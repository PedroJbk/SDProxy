...
/// Handle SECURITY - SEGUE O PADRÃO EXATO DO HTTP INJECTOR com [split]:
/// 1. Recebe parte 1 do payload
/// 2. Envia 101
/// 3. Recebe parte 2 do payload
/// 4. Envia 200
/// 5. Conecta ao backend
/// 6. Tunnel bidirecional
pub async fn handle_security(mut socket: TcpStream, status: &str) -> Result<()> {
    info!("🔐 SECURITY handshake (padrão HTTP Injector [split])...");
    ...
    info!("📥 SECURITY parte 1 ({} bytes): {:?}", n1, &part1[..std::cmp::min(n1, 200)]);

    // PASSO 2: Envia 101 Switching Protocols
    ...
    info!("📥 SECURITY parte 2 ({} bytes): {:?}", n2, &part2[..std::cmp::min(n2, 200)]);
