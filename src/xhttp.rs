...
/// Handle xHTTP (SplitHTTP) - PADRAO BSProxy
/// Para porta 443 com TLS passthrough
pub async fn handle_xhttp(mut socket: TcpStream, status: &str) -> Result<()> {
    info!("xHTTP/SplitHTTP handler...");

    // Ler payload do Injector
    let mut buf = [0u8; 8192];
    let n = socket.read(&mut buf).await?;
    info!("xHTTP payload recebido ({} bytes)", n);

    if n == 0 {
        return Ok(());
    }

    // Detectar backend pelo payload
    let payload_str = String::from_utf8_lossy(&buf[..n]);
    let addr_proxy = if payload_str.contains("SSH") || payload_str.contains("ssh") || payload_str.is_empty() {
        "127.0.0.1:22"
    } else {
        "127.0.0.1:1194"
    };
    ...
    // Conectar ao backend
    info!("xHTTP -> conectando ao backend: {}", addr_proxy);
    let server_stream = match TcpStream::connect(addr_proxy).await {
        ...
        Err(e) => {
            ...
            info!("xHTTP falha em {}: {}. Tentando {}", addr_proxy, e, alt);
            match TcpStream::connect(alt).await {
                Ok(s) => s,
                Err(e2) => {
                    info!("xHTTP ambos falharam: {}, {}", e, e2);
                    return Ok(());
                }
            }
        }
